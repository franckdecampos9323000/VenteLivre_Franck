    ...
    <div class="d-flex flex-column flex-column-fluid">
        ...
        <div id="kt_app_toolbar" class="app-toolbar pt-6 pb-2">
            ...
            <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex align-items-stretch">
                ...
                <div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100">
                    <!--begin::Page title-->
                    <x-title :pageTitle="'Liste des utilisateurs'" />
                    <!--begin::Page title-->
                    <!--begin::Breadcrumb-->
                    <x-breadcrumb
                        :link1="route('home')"
                        ...
                        :text2="'Utilisateurs'"
                        :link3="route('admin.users.index')"
                        :text3="'Liste des utilisateurs'"
                        :link4="''"
                        :link5="''"
                    />
                    <!--End::Breadcrumb->
                    <!--begin::Button-->
                    <x-add-button :link="route('admin.users.create')" :name="'un utilisateur'" />
                    <!--begin::Button-->
                </div>
                <!--end::Toolbar wrapper-->
            </div>
        ...
        <div id="kt_app_content" class="app-content flex-column-fluid">
            ...
            <div id="kt_app_content_container" class="app-container container-fluid">
                <!--begin::Row-->
                <x-stat-list
                    :icon1="'fa fa-users fs-2 me-2'"
                    :texticon1="'Nombre d\'utilisateurs'"
                    :text1="$totalUsers"
                    :icon2="'fas fa-check-circle fs-2 text-success me-2'"
                    :texticon2="'Utilisateur actif'"
                    :text2="$activeCount"
                    :icon3="'fas fa-times-circle fs-2 me-2'"
                    :texticon3="'Utilisateur non actif'"
                    :text3="$inactiveCount"
                />
                <!--end::Row-->
                <br>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="symbol symbol-45px me-5">
                                    <img src="{{ asset($user->getFirstMediaUrl('photo_profils')) }}" alt="{{ $user->username }}" class="img-fluid">
                                </div>
                            </td>
                            <td> {{ $user->email}} </td>
                            <td> {{ $user->username}} </td>
                            ...
                            <td>
                                <form action="{{ route('users.toggleStatus', $user->id) }}" method="POST">
                                    ...
                                    @method('PATCH')
                                    <!--begin::Switch-->
                                    <label for="status_{{ $user->id }}" class="form-check form-switch form-check-custom form-check">
                                        <!--begin::Input-->
                                        <input class="form-check-input w-30px h-20px" type="checkbox"  name="status" {{ $user->status ? 'checked' : '' }} onchange="this.form.submit()"/>
                                        <!--end::Input-->
                                        <!--end::Label-->
                                        @if($user->status == true)
                                        ...
                                        @endif
                                        <!-- <span class="form-check-label text-muted fs-6">{{ $user->status ? 'Activé' : 'Désactivé' }}</span> -->
                                        <!--end::Label-->
                                    </label>
                                    <!--end::Switch-->
                                </form>
                            ...
                            <td>
                                <a href="{{ route('admin.users.edit', $user) }}" class="btn btn-icon btn-active-color-primary btn-sm me-1" data-bs-toggle="tooltip" title="Modifier l'utilisateur">
                                ...
                                <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" class="btn btn-icon btn-active-color-primary btn-sm" onclick="deleteline(event)" data-bs-toggle="tooltip" title="Supprimer l'utilisateur">
