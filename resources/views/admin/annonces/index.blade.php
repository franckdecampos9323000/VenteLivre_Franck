    ...
    <div class="d-flex flex-column flex-column-fluid">
        ...
        <div id="kt_app_toolbar" class="app-toolbar pt-6 pb-2">
            ...
            <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex align-items-stretch">
                ...
                <div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100">
                    <!--begin::Page title-->
                    <x-title :pageTitle="'Liste des annonces'" />
                    <!--begin::Page title-->
                    <!--begin::Breadcrumb-->
                    <x-breadcrumb
                        :link1="route('home')"
                        :link2="route('admin.annonces.index')"
                        :text2="'Annonces'"
                        :link3="route('admin.annonces.index')"
                        :text3="'Liste des annonces'"
                        :link4="''"
                        :link5="''"
                    />
                    <!--End::Breadcrumb->
                    <!--begin::Button-->
                    <x-add-button :link="route('admin.annonces.create')" :name="'une annonce'" />
                    <!--begin::Button-->
                </div>
                <!--end::Toolbar wrapper-->
            </div>
        ...
        <div id="kt_app_content" class="app-content flex-column-fluid">
            ...
            <div id="kt_app_content_container" class="app-container container-fluid">
                <!--begin::Row-->
                <x-stat-list
                    :icon1="'fa fa-graduation-cap fs-2 me-2'"
                    :texticon1="'Nombre de annonces'"
                    :text1="$totalAnnonces"
                    :icon2="'fas fa-check-circle fs-2 text-success me-2'"
                    :texticon2="'annonce activée'"
                    :text2="$activeCount"
                    :icon3="'fas fa-times-circle fs-2 me-2'"
                    :texticon3="'annonce désactivée'"
                    :text3="$inactiveCount"
                />
                <!--end::Row-->
                <br>
                ...
                <!--end::Alert-->
                @endif
                <table id="kt_datatable_dom_positioning_row3" class="table table-striped table-row-bordered gy-5 gs-7 border rounded">
                    <thead>
                        <tr class="fw-bold fs-6 text-gray-800 px-7">
                            ...
                            <th>Auteur</th>
                            <th>Statut <!-- / ALL
                            <form id="annonceSwitchForm" action="{{ route('annonces.allStatus') }}" method="POST">
                                @csrf
                                @method('PATCH')
                                <label class="form-switch form-check">
                                    <input class="form-check-input" type="checkbox" value="1" name="allstatus" {{ $annonces->pluck('allstatus')->contains(true) ? 'checked' : '' }} onchange="submitForm()" />
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <img src="{{ asset('images/' . $annonce->image) }}" alt="{{ $annonce->title }}" class="img-fluid">
                                <!-- <img src="{{ asset($annonce->getFirstMediaUrl('imagest')) }}" alt="{{ $annonce->title }}" class="img-fluid"> -->
                                </div>
                            </td>
                            <td><a class="text-dark fw-bold text-hover-primary mb-1 fs-6" href="{{ route('admin.annonces.show', $annonce) }}">{{ $annonce->title }}</a>  </td>
                            <td> {{ $annonce->user->username}} </td>
                            <td>
                                <form action="{{ route('annonces.toggleStatus', $annonce->id) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                                    <!--begin::Switch-->
                                    <label for="status_{{ $annonce->id }}" class="form-check form-switch form-check-custom form-check">
                                        <!--begin::Input-->
                                        <input class="form-check-input w-30px h-20px" type="checkbox"  name="status" {{ $annonce->status ? 'checked' : '' }} onchange="this.form.submit()"/>
                                        <!--end::Input-->
                                        <!--end::Label-->
                                        @if($annonce->status == true)
                                        ...
                                        @endif
                                        <!-- <span class="form-check-label text-muted fs-6">{{ $annonce->status ? 'Activé' : 'Désactivé' }}</span> -->
                                        <!--end::Label-->
                                    </label>
                                    <!--end::Switch-->
                                </form>
                            ...
                            <td>
                                <a href="{{ route('admin.annonces.show', $annonce) }}" class="btn btn-icon btn-active-color-primary btn-sm me-1" data-bs-toggle="tooltip" title="Voir plus de détail">
                                    <!--begin::Svg Icon | path: assets/media/icons/duotune/general/gen015.svg-->
                                ...
                                <a href="{{ route('admin.annonces.edit', $annonce) }}" class="btn btn-icon btn-active-color-primary btn-sm me-1" data-bs-toggle="tooltip" title="Modifier l' annonce">
                                ...
                                <form action="{{ route('admin.annonces.destroy', $annonce->id) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" class="btn btn-icon btn-active-color-primary btn-sm" onclick="deleteline(event)" data-bs-toggle="tooltip" title="Supprimer l' annonce">
